...
most_spoken_languages = sorted_languages[:10]
...
for data in countries():
    ...
    population_data.append(country_info)
sorted_population = sorted(population_data, key=lambda country: country['population'], reverse=True)
most_populated_countries = sorted_population[:10]
for rank, country in enumerate(most_populated_countries, 1):
    formatted_population = f"{country['population']:,}"
    print(f"{rank}. {country['name']}: {formatted_population}")
